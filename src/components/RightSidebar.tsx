...
const ExpandableCard = ({ title, icon: Icon, children, defaultExpanded = true }: any) => {
  ...
  return (
    <div className="bg-maritime-900/90 backdrop-blur-xl border border-slate-800/60 rounded-2xl shadow-2xl overflow-hidden pointer-events-auto flex flex-col transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]">
      <button 
        onClick={() => setIsExpanded(!isExpanded)}
        className="flex items-center justify-between p-5 hover:bg-slate-800/20 transition-colors w-full group"
      >
        <div className="flex items-center gap-3">
          <Icon size={16} className="text-cyan group-hover:scale-110 transition-transform duration-300" />
          <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 group-hover:text-white transition-colors">{title}</h3>
        </div>
...
const RightSidebar = () => {
  return (
    <div className="fixed right-6 top-6 bottom-6 w-96 flex flex-col gap-4 pointer-events-none z-[1500] overflow-y-auto custom-scrollbar pr-1">
      ...
      <ExpandableCard title="Weather Conditions" icon={Activity}>
        <WeatherTelemetry />
      </ExpandableCard>

      <ExpandableCard title="Tidal Forecast" icon={Waves}>
      ...
      <div className="mt-auto pointer-events-auto">
        <div className="bg-maritime-900/80 backdrop-blur-md border border-slate-800/40 px-6 py-3 rounded-2xl shadow-xl flex items-center justify-between group hover:border-cyan/30 transition-all">
          ...
          <span className="text-[10px] font-mono text-cyan font-bold tracking-widest group-hover:drop-shadow-[0_0_5px_rgba(0,242,255,0.5)] transition-all">9447427</span>
        </div>
      </div>
    </div>
  );
